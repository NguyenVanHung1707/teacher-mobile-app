...
export default function ClassDocuments({ classId }) {
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      const token = await getData('accessToken');
      await axios.post(`${API_URL}/documents/folder`, {
        courseId: classId,
          ...
          { 
            ...
            onPress: async () => {
              ...
              try {
                ...
                Alert.alert('Thành công', `Tệp "${doc.name}" đã được lưu về bộ nhớ qua bộ truyền dữ liệu.`);
              }
            } 
          }
        ]
        ...
        <View style={styles.leftContainer}>
          ...
          <View style={styles.textContainer}>
            ...
            <Text style={styles.docDetails}>
              {isFolder ? 'Thư mục' : `${formatBytes(item.fileSize)} • bởi ${item.uploaderName || 'Giảng viên'}`}
            </Text>
          </View>
        </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.quickActions}>
        <TouchableOpacity style={styles.actionBtn} onPress={() => setIsFolderModalVisible(true)}>
          <Icon name="folder-open" size={14} color="#FFF" style={{ marginRight: 6 }} />
          <Text style={styles.actionBtnText}>Thư mục mới</Text>
        </TouchableOpacity>

        <TouchableOpacity style={[styles.actionBtn, styles.uploadBtn]} onPress={handleUpload} disabled={uploading}>
          {uploading ? (
      ...
      {loading ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>
            {searchQuery ? 'Không tìm thấy tài liệu phù hợp.' : 'Thư mục trống. Hãy tạo thư mục hoặc tải tài liệu lên!'}
          </Text>
        </View>
      ) : (
        <FlatList
